...
export default function TimeFunnelTechnicianToFinancePage() {
  const now = new Date();
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const response = await axiosClient.get<PaginatedResponse<TimeFunnelRow>>('/leads/timeFunnel/technicianToFinance', {
        params: {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-gray-900">Časový funnel – Technik → FŘ</h1>
      <p className="text-sm text-gray-600 mt-1">
        Jak číst: každý řádek je lead, který měl status „Předáno technikovi“ (UPLOAD_DOCUMENTS) v zvoleném období. “Doba” je čas od
        prvního „Technik“ po první “Předáno k vyplacení” (FINAL_APPROVAL). Pokud ještě není předáno FŘ, zobrazuje se stáří od “Technik” do teď.
      </p>

      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700">Od (Technik)</label>
          <input
            type="date"
            value={dateFrom}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Do (Technik)</label>
          <input
            type="date"
            value={dateTo}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Obchodník (kdo předal FŘ)</label>
          <select
      ...
      <div className="mt-4 text-sm text-gray-700">
        <span className="mr-4">
          Dokončeno (předáno FŘ): <span className="font-semibold">{summary.count}</span>
        </span>
        <span className="mr-4">
                ...
                {(data?.results ?? []).map((row) => (
                  <tr key={row.leadId} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900 font-medium">
                      {formatDuration(row.durationMinutes ?? row.ageMinutes)}
                    </td>
                  </tr>
                ))}
