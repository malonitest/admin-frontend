...
export default function TimeFunnelNewToDecisionPage() {
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const response = await axiosClient.get<PaginatedResponse<TimeFunnelRow>>('/leads/timeFunnel/newToDecision', {
        params: {
  ...
  const summary = useMemo(() => {
    ...
    const median = count
      ? durations[Math.floor((count - 1) / 2)]
      : null;
  ...
  return (
    <div className="p-6">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Obchodník (kdo rozhodl)</label>
          <select
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Změnil status</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Rozhodnutí</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Zamítnuto</th>
                ...
                {(data?.results ?? []).map((row) => (
                  <tr key={row.leadId} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">
                      {row.decisionBy?.name || row.decisionBy?.email || '-'}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-900">{decisionLabel(row)}</td>
                ...
                {(data?.results?.length ?? 0) === 0 && (
                  <tr>
                    <td className="px-4 py-6 text-sm text-gray-600" colSpan={7}>
                      V tomto období nejsou žádné rozhodnuté leady.
                    </td>
                  </tr>
                )}
              </tbody>
