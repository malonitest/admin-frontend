...
export default function LeadDetailV2() {
  ...
  const miscUploadInputRef = useRef<HTMLInputElement>(null);
  const miscCameraInputRef = useRef<HTMLInputElement>(null);
  const [miscDragActive, setMiscDragActive] = useState(false);
  ...
  const hasMiscDocuments = useMemo(() => {
    const items = (lead?.documents?.misc || []) as any[];
    return Array.isArray(items) && items.some((d) => d && typeof d === 'object' && typeof d.file === 'string' && d.file);
  }, [lead]);
  ...
  const uploadMiscDocuments = useCallback(
    async (files: File[]) => {
      if (!id) return;
      if (!files || files.length === 0) return;

      const existing = (lead?.documents?.misc || []) as LeadDocument[];
      const existingCount = Array.isArray(existing)
        ? existing.filter((d) => d && typeof d === 'object' && typeof d.file === 'string' && d.file).length
        : 0;
      ...
      setUploadingPhotoKey('misc');
      try {
        for (const file of toUpload) {
          const formData = new FormData();
          formData.append('document', file);
          // Backend category for other documents
            ...
            >
              ...
              <div className="p-4">
                {documentsView === 'categories' ? (
                    ...
                    {[
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (label === 'Ostatni dokumenty') {
                            setDocumentsView('misc');
                          }
                        }}
                  ...
                  <>
                    <div className="text-sm font-medium text-gray-800 mb-3">Ostatní dokumenty</div>

                    <div
                      className={`border-2 border-dashed rounded-lg p-4 mb-4 transition-colors ${
                        miscDragActive ? 'border-red-600 bg-red-50' : 'border-gray-300 bg-gray-50'
                      }`}
                      onDragOver={(e) => {
                        e.preventDefault();
                        setMiscDragActive(true);
                      }}
                      onDragLeave={() => setMiscDragActive(false)}
                      onDrop={async (e) => {
                        e.preventDefault();
                        setMiscDragActive(false);
                        const files = Array.from(e.dataTransfer.files || []);
                        await uploadMiscDocuments(files);
                      }}
                    >
                      <input
                        ref={miscUploadInputRef}
                        type="file"
                        accept="image/*,application/pdf,.pdf"
                        multiple
                        onChange={async (e) => {
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        <button
                          type="button"
                          onClick={() => miscUploadInputRef.current?.click()}
                          disabled={uploadingPhotoKey === 'misc'}
                          className="flex-1 w-full py-3 px-4 bg-gray-100 hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg text-gray-700"
                        >
                          {uploadingPhotoKey === 'misc' ? 'Nahrávám...' : 'Nahrát (PDF/foto)'}
                        </button>
                        <button
                          type="button"
                          onClick={() => miscCameraInputRef.current?.click()}
                          disabled={uploadingPhotoKey === 'misc'}
                          className="flex-1 w-full py-3 px-4 bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg text-white"
                        >
                          Vyfotit
                      ...
                      <p className="text-xs text-gray-500 text-center mt-3">
                        {miscDragActive ? 'Pusťte soubory zde…' : 'nebo přetáhněte soubory sem (drag & drop)'}
                      </p>
                      <p className="text-xs text-gray-500 text-center mt-1">Limit: 10 dokumentů (obrázky + PDF)</p>
                    </div>

                    {Array.isArray(lead?.documents?.misc) &&
                    (lead?.documents?.misc || []).some((d) => typeof (d as any)?.file === 'string' && (d as any).file) ? (
                      <div className="grid grid-cols-1 gap-2">
                        {(lead?.documents?.misc || [])
                          .filter((d) => typeof (d as any)?.file === 'string' && (d as any).file)
                          .slice(0, 10)
